...
function * run() {
	...
	function getImage(rows)
	{
		...
		setTimeout(function(){
			console.log(index + " of "+ allRows.length + " : " +row.imdb);
			adapterFor(row.url).get(row.url, function(data){
			    ...
			    {
					var fileDest = 'results/imdb/poster_new/'+ row.imdb + '.jpg';
	...
	function getMoviesFromDb()
	{
		db.serialize(function() {
			db.all("SELECT id, imdb, image_location_high AS url FROM " + table + " WHERE image_location_high NOT NULL AND image_location_high != 'N/A'", function(err, rows) {
				allRows = rows;
